...
/* ──────────────────────────────────────────────────────────────
   7. tweakpane.js — per-country physics config
   ────────────────────────────────────────────────────────────── */
describe('tweakpane per-country config', () => {
  let initTweakpane;
  let mockStorage;
  let origGetItem;
  let origSetItem;
  ...
  it('exposes applyCountryConfig and saveCountryConfig', () => {
    initTweakpane({ rerender: () => {}, onCountryChange: () => {} });
    expect(typeof initTweakpane.applyCountryConfig).toBe('function');
    expect(typeof initTweakpane.saveCountryConfig).toBe('function');
  });

  it('applyCountryConfig resets to defaults when no overrides exist', () => {
    const CONFIG = initTweakpane({ rerender: () => {}, onCountryChange: () => {} });
    CONFIG.gravity = 1.5;
    CONFIG.damping = 0.8;
    initTweakpane.applyCountryConfig('japan');
    expect(CONFIG.gravity).toBe(0.2);
    expect(CONFIG.damping).toBe(0.99);
  });

  it('applyCountryConfig applies saved overrides on top of defaults', () => {
    const CONFIG = initTweakpane({ rerender: () => {}, onCountryChange: () => {} });
    CONFIG.gravity = 0.5;
    initTweakpane.saveCountryConfig();
    CONFIG.gravity = 0.2;
    initTweakpane.applyCountryConfig('china');
    expect(CONFIG.gravity).toBe(0.5);
    expect(CONFIG.damping).toBe(0.99);
  });

  it('country switch: previous country saved, new country loaded', () => {
    const CONFIG = initTweakpane({ rerender: () => {}, onCountryChange: () => {} });
    CONFIG.gravity = 1.5;
    initTweakpane.saveCountryConfig();
    initTweakpane.applyCountryConfig('japan');
    expect(CONFIG.gravity).toBe(0.2);
    initTweakpane.applyCountryConfig('china');
    expect(CONFIG.gravity).toBe(1.5);
  });

  it('independent configs per country do not interfere', () => {
    const CONFIG = initTweakpane({ rerender: () => {}, onCountryChange: () => {} });
    CONFIG.gravity = 0.5;
    CONFIG.damping = 0.95;
    initTweakpane.saveCountryConfig();
    ...
    initTweakpane.applyCountryConfig('japan');
    expect(CONFIG.gravity).toBe(1.8);
